...
class continuumCPA(object):
	...
	self.omega_set = list(np.linspace(self.omega_min,self.omega_max,1001))
        ...
        self.d_k_lin = k_values[1] - k_values[0]
        ...
        for idx_k in range(0,50):
            ...
            integ_check += self.d_k_lin*(self.k_set[-1])**2
	...
	guess = self.compute_initial_guess()
        ...
        for omega in self.omega_set:
            self.z.append(complex(-(omega**2),self.epsilon)*(omega**2))
    ...
    def compute_modified_greens_function(self,z,omega,idx):
        ...
        for k_x in self.k_set:
            self.tilde_G_z[idx] += self.d_k_lin*(k_x**2)/(z+self.Gamma[idx]*k_x**2)
        
        self.tilde_G_z[idx] *= (3.0/(self.k_max**3))
	...
	for idx in idset:
		...
		self.convergence[idx] = True
            #print 'z: ', self.z[idx]
            print 'Gamma ', idx, ' calculated: ', self.tilde_G_z[idx], self.count
            sys.stdout.flush()
	    if idx<idset[-1]:
