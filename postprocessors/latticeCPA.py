...
class latticeCPA(object):
	...
	self.omega_set = list(np.linspace(self.omega_min,self.omega_max,251))
        ...
        # K values range
	size = 24
        k_values = np.linspace(-math.pi,math.pi,size+1)
	...
	while (abs(update.imag))>1e-12 and self.count<100:
...
#            denominator += (1.0/factor)*self.K_hist[this_key]
	...
	for idx in range(len(self.omega_set)-1,-1,-1):
	    ...
	    #self.g_omega.append(-2.0*self.tilde_G_z[omega_idx].imag*self.omega_set[omega_idx]/(math.pi*self.z[omega_idx]))
            ...
            self.mean_free_path.append(modulus**2/denominator)
    ...
    def write_DoS_MFP(self):
        ...
        for omega_idx in xrange(0,len(self.omega_set)):
            print >> self.phonon_file, self.omega_set[omega_idx],',',self.g_omega[omega_idx],',',self.mean_free_path[omega_idx],',',self.wavelength[omega_idx],',',abs(self.sound_velocity[omega_idx])
