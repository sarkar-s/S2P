...
def get_diff_value(last_idx,this_time,time_set,diff_set):
    diff_value = 0.0
    
    for idx in xrange(last_idx,len(time_set)-1):
        if this_time>=time_set[idx] and this_time<time_set[idx+1]:
            diff_value = diff_set[idx]
            break
    
    return diff_value, idx
...
if __name__ == '__main__':
    ...
    data_tag = 'Network Diffusivity'
        
    outfile = open('diffusivity_summary.csv','w')
    
    diff_mean = np.zeros(shape=(data_size+1))
    t_set = []
    
    for i in xrange(0,data_size):
        t_set.append(math.exp(l_t_set[i]))
    
    diff_mean[0] =  float(good_file_count)
    ...
    for t_no in range(0,data_size-1):
        ...
        for k in range(0,good_file_count):
            diff_value, last_idx =  get_diff_value(t_idx[k],this_time,timeDC_data[k]['Time'],diff_data[k][data_tag])
            
            diff_mean += diff_value
            t_idx[k] = last_idx
    
        print >> outfile, str(t_set[t_no])+','+str(diff_mean/good_file_count)
