...
if __name__ == '__main__':
    ...
    for file_no in range(0,samples):
        try:
            ...
            chainstats_file = open('site_fractions'+str(file_no)+'.csv','r')
    ...
    data_tags = ['Empty sites','Surface sites','Blocked sites']
        
    outfile = open('sites_summary.csv','w')
    
    print >> outfile, 'Time,DC,Empty sites,Surface sites,Blocked sites, Total effected sites'
    ...
    for t_no in range(0,1001):
        total_effected = (surface_mean[t_no]+blocked_mean[t_no])/float(good_file_count)
        
        outstring = str(t_set[t_no])+','+str(dc_mean[t_no]/good_file_count)+','+str(empty_mean[t_no]/good_file_count)+','+str(surface_mean[t_no]/good_file_count)+','+str(blocked_mean[t_no]/good_file_count)+','+str(total_effected)
